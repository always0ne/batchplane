...
function MyWorkContent({ state }: { state: MyWorkState }) {
  const { t } = useTranslation("myWork");
...
function toWorkRow(item: MyWorkItem): WorkRow {
...
function countWorkItems(items: WorkRow[]): Record<WorkKind, number> {
  return {
...
function compareWorkItems(left: WorkRow, right: WorkRow) {
...
function formatWorkTime(value: string, fallback: string) {
