...
export type RegistrationRequestMode = "create" | "change";
...
export function buildRegistrationPullRequestTitle(
  ...
  return `${mode === "change" ? "Change" : "Register"} batch ${definition.batchId}`;
}
...
export function buildRegistrationPullRequestBody(
  ...
  const heading =
    mode === "change" ? "## BatchPlane Change" : "## BatchPlane Registration";
  const requestType = mode === "change" ? "CHANGE" : "REGISTER";
  const summary =
    mode === "change"
      ? "This pull request was generated by BatchPlane Lite and updates the governed batch definition and its GitHub Actions workflow."
