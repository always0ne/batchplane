...
function validateBatchDefinitionFile(file) {
  ...
  if (!isBatchPlaneApiVersion(file.apiVersion) || file.kind !== "BatchDefinition") {
    return { ok: false };
  }
...
function validateWorkspacePolicyFile(file) {
  ...
  if (!isBatchPlaneApiVersion(file.apiVersion) || file.kind !== "WorkspacePolicy") {
    return { ok: false };
  }
  const metadata = asRecord(file.metadata);
  const spec = asRecord(file.spec);
...
function readOptionalRepositoryRolesArray(value) {
  ...
  if (!Array.isArray(value) || value.some((item) => !repositoryRoleValues.includes(item))) {
    return { ok: false };
  }
  return { ok: true, value };
}
...
async function verifyLiteAuthorization(input) {
  ...
  if (evidence.approval.approvalType === "SCHEDULE_DELEGATED") {
    ...
    return {
      result: "ALLOW",
      message: "Scheduled execution request, delegated approval evidence, and batch policy are verified."
    };
  }
