...
export async function verifyLiteAuthorization(input) {
    ...
    if (evidence.approval.approvalType === "SCHEDULE_DELEGATED") {
        if (evidence.request.triggerType !== "SCHEDULE" ||
            !evidence.request.scheduleId) {
            return deny("SCHEDULE_DELEGATED_APPROVAL_INVALID", "Delegated schedule approval requires a scheduled execution request.");
        }
        if (evidence.approval.approver !== expectedActor) {
            return deny("SCHEDULE_DELEGATED_APPROVER_INVALID", `Delegated schedule approval must be recorded by ${expectedActor}.`);
        }
        return {
            result: "ALLOW",
...
async function validateBatchPolicyEvidence({ batchId, client, configPath, inputRef, repository, request, }) {
    ...
    if (request.triggerType === "SCHEDULE") {
        if (!request.scheduleId) {
            return deny("SCHEDULE_NOT_MAPPED", "Scheduled execution request does not contain a schedule identifier.");
        }
        if (!snapshot.enabledScheduleIds.includes(request.scheduleId)) {
            return deny("SCHEDULE_NOT_REGISTERED", `Schedule ${request.scheduleId} is not enabled in batch ${batchId}.`);
        }
    }
    if (!effectiveRef) {
...
function parseExecutionApprovalEvidence(comment) {
    ...
    const approvalType = marker.get("approvalType") ??
...
function parseBatchDefinitionSnapshot(content) {
    ...
    return {
        enabledScheduleIds: value.spec.schedules
