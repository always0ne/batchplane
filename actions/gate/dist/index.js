...
async function verifyManualAuthorization({
  ...
  if (approval.approver === request.requestedBy && !allowsSelfApproval2(workspaceApprovalMode)) {
    return deny(
  ...
  const approverAuthorized = await verifyApproverAuthorization({
    allowMissingRoleMapping: approval.approver === request.requestedBy && allowsSelfApproval2(workspaceApprovalMode),
...
function allowsSelfApproval2(mode) {
