...
var batchPlaneDispatcherActionRef = "always0ne/batchplane/actions/dispatcher@main";
...
var scheduleCronSampleStart = /* @__PURE__ */ new Date("2026-01-01T00:00:00.000Z");
var scheduleCronOccurrenceSampleSize = 512;
var scheduleTimezoneOffsetSampleDates = [
  /* @__PURE__ */ new Date("2026-01-01T12:00:00.000Z"),
  /* @__PURE__ */ new Date("2026-04-01T12:00:00.000Z"),
  /* @__PURE__ */ new Date("2026-07-01T12:00:00.000Z"),
  ...
  const scheduleEntries = Array.from(
    new Map(
      enabledSchedules.flatMap(
        (schedule) => getGeneratedScheduleCrons(schedule).map((entry) => ({
          cron: entry.cron
        }))
      ).filter((schedule) => schedule.cron).map((schedule) => [schedule.cron, schedule])
    ).values()
  );
  return [
...
function getGeneratedScheduleCrons(schedule) {
...
function parseGitHubCronFields(cron) {
...
function convertDailyCronToUtcCronEntries(fields, timezone) {
  ...
  const minuteHourPairs = /* @__PURE__ */ new Set();
...
function convertCronOccurrencesToUtcCronEntries(cron, timezone, fields) {
  const interval = import_cron_parser.CronExpressionParser.parse(cron, {
    currentDate: scheduleCronSampleStart,
    tz: timezone
  });
  ...
  for (let index = 0; index < scheduleCronOccurrenceSampleSize; index += 1) {
    ...
    } else if (fields.dayOfMonth !== "*" && fields.month === "*" && fields.dayOfWeek === "*") {
...
function formatMinuteHourCronEntries(pairs) {
  const minutesByHour = /* @__PURE__ */ new Map();
  for (const pair of pairs) {
    const [minuteText = "", hourText = ""] = pair.split(" ");
    const minute = Number(minuteText);
    const hour = Number(hourText);
    const minutes = minutesByHour.get(hour) ?? /* @__PURE__ */ new Set();
    minutes.add(minute);
    minutesByHour.set(hour, minutes);
  }
  return [...minutesByHour.entries()].sort(([left], [right]) => left - right).map(
    ([hour, minutes]) => `${formatCronNumberList(
      [...minutes].sort((left, right) => left - right)
    )} ${hour} * * *`
  );
}
function expandSimpleCronField(field, min, max) {
...
function getObservedTimezoneOffsets(timezone) {
...
function getTimezoneOffsetMinutes(date, timezone) {
  ...
  return Math.round((zonedAsUtc - date.getTime()) / 6e4);
}
...
function formatGitHubScheduleMatchExpression(crons) {
  return crons.map((cron) => `github.event.schedule == ${githubExpressionString(cron)}`).join(" || ");
}
function compareCronText(left, right) {
...
function validateBatchDefinitionFile2(file) {
  ...
  if (!isBoolean(spec.gateRequired) || !isAllowedBatchStatus(spec.status) || !isString(workflow.path) || !isString(workflow.ref)) {
    return { ok: false };
  }
  return {
    ok: true,
    value: {
      ...
      spec: {
        gateRequired: spec.gateRequired,
        schedules: Array.isArray(spec.schedules) ? spec.schedules.map(asRecord).filter(
          (schedule) => schedule !== null && isString(schedule.id) && isBoolean(schedule.enabled)
        ).map((schedule) => ({
...
function validateWorkspacePolicyFile2(file) {
  ...
  if (!metadata || !spec || !approval || !isString(metadata.id) || !isWorkspaceApprovalMode(approval.mode)) {
    return { ok: false };
  }
  return {
    ok: true,
...
async function verifyLiteAuthorization(input, verifyBatchRevision = verifyApprovedBatchRevision) {
  ...
  if (evidence.approval.commandDigest && evidence.approval.commandDigest !== evidence.request.requestDigest) {
    return deny(
      "REQUEST_DIGEST_MISMATCH",
      "Approval command digest does not match execution request digest."
    );
  }
  if (evidence.approval.requestDigest !== input.requestDigest || evidence.approval.requestDigest !== evidence.request.requestDigest) {
    return deny(
      "REQUEST_DIGEST_MISMATCH",
  ...
  if (evidence.approval.approvalType === "SCHEDULE_DELEGATED") {
    if (evidence.request.triggerType !== "SCHEDULE" || !evidence.request.scheduleId) {
      return deny(
    ...
    successMessage = "Scheduled execution request, delegated approval evidence, and batch policy are verified.";
  } else {
    ...
    if (evidence.approval.approvalType === "WORKSPACE_AUTO_APPROVED") {
      ...
      successMessage = "Execution request, Workspace auto-approval evidence, and batch policy are verified.";
    } else {
      if (evidence.approval.approver === evidence.request.requestedBy && !allowsSelfApproval(workspaceApprovalMode)) {
        return deny(
          "SELF_APPROVAL_NOT_ALLOWED",
          "Requester and approver must be different users."
        );
      }
      const selfApprovalAllowedWithoutRoleMapping = evidence.approval.approver === evidence.request.requestedBy && allowsSelfApproval(workspaceApprovalMode);
      ...
      if (!approverAuthorized.allowed) {
        return deny(
          ...
          approverAuthorized.message || `Approver @${evidence.approval.approver} is not authorized.`
        );
      }
      successMessage = "Execution request, approval evidence, and batch policy are verified.";
    }
  }
...
async function findGitHubApprovalEvidence({
  ...
  const approval = comments.map(parseExecutionApprovalEvidence).find(
...
if (process.env.GITHUB_ACTIONS === "true" && process.env.BATCHTRAIL_GATE_DISABLE_AUTO_RUN !== "true") {
