...
function createRepositoryOperations(requester) {
...
function createPullRequestOperations(requester) {
  const { request } = requester;
  return {
...
function createIssueOperations(requester) {
  const { request } = requester;
  return {
...
function createWorkflowOperations(requester) {
  ...
  return {
    ...
    async getWorkflowRun({ owner, repo, runAttempt, runId }) {
      const run = await request(runAttempt ? `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/actions/runs/${runId}/attempts/${runAttempt}` : `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/actions/runs/${runId}`, {}, { allowNotFound: true });
...
function createRepositoryAccessOperations(requester) {
  const { request } = requester;
  return {
...
function validateBatchDefinitionFile(file) {
  ...
  if (metadata && spec) {
    const schedules = Array.isArray(spec.schedules) ? spec.schedules.map((schedule) => {
...
// ../../packages/github-lite/dist/native-schedule-evidence.js
...
function parseMarker(body, name) {
  ...
  const match = body.match(new RegExp(`<!--\\s*(?:batchplane|batchtrail):${name}\\s*([\\s\\S]*?)-->`, "u"));
  if (!match?.[1])
    return marker;
  for (const line of match[1].split("\n")) {
    const separator = line.indexOf("=");
    if (separator > 0) {
      marker.set(line.slice(0, separator).trim(), line.slice(separator + 1).trim());
    }
  }
  return marker;
}
...
function verifyLiteInput(input) {
  if (input.mode !== "lite") {
    return deny2("UNSUPPORTED_MODE", "Only lite mode is scaffolded.");
  }
  if (!input.batchId) {
    return deny2("BATCH_ID_REQUIRED", "Batch ID is required.");
  }
  ...
  if (!input.requestId) {
    return deny2(
      ...
      input.controllerReason ? `Native schedule controller denied this occurrence: ${input.controllerReason}.` : "Execution request evidence is required."
    );
  }
...
function deny2(reasonCode, message) {
  return { message, reasonCode, result: "DENY" };
}
...
// src/gate-native-authorization.ts
...
function deny4(reasonCode, message) {
  return { message, reasonCode, result: "DENY" };
}
...
async function loadApprovalEvidence(input, client) {
  try {
    ...
    return deny5(
      ...
      `GitHub evidence lookup failed: ${toErrorMessage2(error)}`
    );
  }
}
function verifyRequestEvidence(input, evidence) {
  const request = evidence.request;
  if (!request) {
    return deny5(
  ...
  if (request.requestId !== input.requestId || request.batchId !== input.batchId || request.requestDigest !== input.requestDigest) {
    return deny5(
      "REQUEST_EVIDENCE_MISMATCH",
  ...
  if (input.eventName !== "schedule" && input.approvalRef !== request.requestId) {
    return deny5(
...
function deny5(reasonCode, message) {
  return { message, reasonCode, result: "DENY" };
}
...
async function runGateFromEnv(env = process.env) {
  ...
  if (input.recordEvidence) {
    try {
      ...
      const message = `Gate decision evidence could not be recorded: ${toErrorMessage3(error)}`;
      if (result.result === "ALLOW") {
        result = deny6("GATE_EVIDENCE_RECORDING_FAILED", message);
      } else {
...
function deny6(reasonCode, message) {
  return { message, reasonCode, result: "DENY" };
}
